// 工作文件夹里给 agent 的文档。模板都在 builtin/workspace，跟着软件走。
...
export async function listAgentDocs(root) {
  return Promise.all((await walk()).map(async (name) => ({
    name,
...
export async function seedAgentDocs(root) {
  for (const name of await walk()) {
    const to = path.join(root, name)
    await fs.mkdir(path.dirname(to), { recursive: true })
    await fs.copyFile(inside(name), to, fs.constants.COPYFILE_EXCL).catch((error) => {
